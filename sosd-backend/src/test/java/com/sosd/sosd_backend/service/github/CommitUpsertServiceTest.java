...
class CommitUpsertServiceTest {
    ...
    void upsertSingleTargetFromResponses_insertsNewAndUpdatesExisting() {
        ...
        var resp1 = new GithubCommitResponseDto("sha-1", 10, 2, "feat: A", "변경 이유 A", now.minusDays(1), now.minusDays(1), null);
        var resp2 = new GithubCommitResponseDto("sha-2", 5, 1, "fix: B", "변경 이유 B", now, now, null);

        // 매퍼 스텁: 응답 -> 업서트 DTO
        var dto1 = new GithubCommitUpsertDto("sha-1", 10, 2, now.minusDays(1).toLocalDateTime(), now.minusDays(1).toLocalDateTime(), "feat: A", "변경 이유 A", null, null, repoId, accountId);
        var dto2 = new GithubCommitUpsertDto("sha-2", 5, 1, now.toLocalDateTime(), now.toLocalDateTime(), "fix: B", "변경 이유 B", null, null, repoId, accountId);
        ...
        assertThat(saved.get(0).getMessage()).isEqualTo("fix: B");
        assertThat(saved.get(0).getMessageBody()).isEqualTo("변경 이유 B");
